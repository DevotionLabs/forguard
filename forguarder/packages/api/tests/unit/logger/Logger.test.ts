...
describe("Logger", () => {
	...
	const testLoggingMethod = (logMethod: (_messageToLog: string) => void, level: string, message: string) => {
		const consoleSpy = jest.spyOn(console, "log").mockImplementation();

		logMethod(message);

		expect(consoleSpy).toHaveBeenCalled();
		expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining(`[${level.toUpperCase()}]`));
